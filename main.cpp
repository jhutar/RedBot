...
struct Options : OptionParser
{
	...
	Options()
		...
		debug(false)
	{
		add("field,f", field_filename, "zadej nazev souboru s popisem hraciho planu");
...
int main(int argc, char **argv) {
	...
	/* bot_dirs - what is working directory of each bot to where store
	files with fights results etc.; if bot's binary is ./binary,
	then bot_dirs="./binary."
	 */
	...
	/* for sudo-mode (bots run by different users),
	    test if we can run bots programs under
	    proper users' accounts */
	...
	for (tid = 0; tid < NUM_PLAYERS; ++tid) {
		...
		if ((last_slash_pos > 2) ||
			...
			bots_bin[tid]=options.strategy[tid].substr(last_slash_pos + 1, options.strategy[tid].size() - 1);
		} else {
		...
		for (i = 0; i < tid; ++i) {
			if (bots_dir[i] == bots_dir[tid]) {
				cerr << "PROBLEM: Dve strategie "
					...
					<< " by si prepsisovaly soubory s informacemi, presunte je do separatnich adresaru."
					<< endl;
				return EXIT_FAILURE;
			}
		}
	...
	do {
		cout << playfield.get_current_round() + 1
			<< ". kolo:";
		/* create & run threads */
		for (tid = 0; tid < NUM_PLAYERS; ++tid) {
			if (playfield.is_kingdom_alive(tid)) {
				thr_data[tid].tid = tid;
				/* binary_to_exe to contain:
					1) in sudo-mode something like "sudo -u botuser0 user0_bot/bot_binary 99 1 2 3 4 5 6 7"
					2) in non-sudo mode (default) then: "user0_bot/bot_binary 99 1 2 3 4 5 6 7"
				 */
				thr_data[tid].binary_to_exe = "cd " + bots_dir[tid] + "; ./" + bots_bin[tid] + " "
					+ unsigned_to_string(playfield.get_remaining_rounds()) + " "
					+ playfield.get_kingdom_details(tid);
				if (options.sudo_mode)
				...
				if (rc) {
					cerr << "pthread_create se neprovedl, navratova hodnota: " << rc << endl;
					return EXIT_FAILURE;
				}
			}
		}
		...
		if ((time_elapsed > ONE_ROUND_DELAY) && (options.debug)) {
                        cout << "       WARN: Nejaka strategie timeoutovala, zadna odpoved behem "
                                << ONE_ROUND_DELAY << "ms." << endl;
                }

		/* evaluate the round */
		playfield.play_one_round(response, bots_dir);
	} while ((!(playfield.game_finished())) && (!(options.one_round)));

	playfield.get_kingdoms_lands(lands);
	cout << "Hra skoncila, kralovstvi maji uzemi:";
	for (i = 0; i < NUM_PLAYERS; i++)
		cout << ' ' << lands[i];
	cout << endl;
